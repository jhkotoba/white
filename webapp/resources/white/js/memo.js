...
let memo = {	
	...
	create : function(id, memoType){
		...
		tag += "<button id='memoAddBt' type='button' class='btn btn-secondary btn-fs nsrb' onclick='memo._add();'>메모 추가</button>";
		tag += "<button id='memoSaveBt' type='button' class='btn btn-secondary btn-fs nsrb' onclick='memo._insert();'>메모 저장</button>";
		tag += "<button id='memoCancelBt' type='button' class='btn btn-secondary btn-fs nsrb' onclick='memo._cancel();'>취소</button>";
	...
	_view : function(){		
		...
		for(let i=0; i<this._list.length; i++){
			...
			tag +=			"<button class='btn btn-secondary btn-fs nsrb' type='button' onclick='memo._delCan("+i+")'>취소</button>";
			tag +=		"</div></div>";			
		}		
		$("#memeList").append(tag);
	},	
	...
	_del : function(idx){
		//새로운 메모 삭제
		switch(memo._list[idx].state){
		
		case "insert" :
			memo._list.splice(idx,1);		
			break;
		case "update" :		
		case "select" :
			memo._list[idx].state = "delete";
			break;
		}	
		this._view();
	},
		...
		if(confirm("메모을 반영 하시겠습니까?")){
			$.ajax({		
			    ...
			    success : function(data, stat, xhr) {  
			    	memo._clone = data;
			    	memo._list = memo._objCopy(memo._clone);
			    	memo._view();
			    	alert("저장이 완료되었습니다.");
			    },
